...
class MatrixActionsDomain extends AbstractDomain {
	...
	protected function saveRecord(string $table, array $record, array $key): int {
		...
		$record["offensive_pool_id"] = $this->getOffensivePoolId($record);
		$record["defensive_pool_id"] = $this->getDefensivePoolId($record);
		$record = $this->finalizeRecord($record, $table);
	...
	protected function getOffensivePoolId(array $record): int {
		...
		// here we want to use the skill, attribute, and limit IDs in our
		// $record to either find or create a pool ID that we return to the
		// calling scope.
		...
		$poolId = $this->db->getVar("SELECT pool_id FROM pools
			WHERE skill_id = :skill_id AND attribute_id = :attribute_id
			AND limit_id = :limit_id AND other_attr_id IS NULL");
