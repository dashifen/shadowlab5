...
abstract class AbstractTransformer implements TransformerInterface {
    ...
    public function transformCreate(PayloadInterface $payload): PayloadInterface {
        ...
        // our goal here is to remove blank, nullable data from our $payload so that
        // it defaults to NULL in our database.  since that's also a goal during an
        // update action, we've separated that behavior into its own method.

        return $this->removeNullableData($payload);
    }
    ...
    public function transformUpdate(PayloadInterface $payload): PayloadInterface {
        ...
        // when we transform for an update, what we're primarily doing is removing
        // the blank information from our $payload so that it can be NULL in our
        // database.  this is the same thing we do when we're creating a new
        // record, so we'll pass control over to that method.
